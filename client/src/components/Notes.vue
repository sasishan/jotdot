...
export default 
{
  ...
  sockets: 
  {
    ...
    SectionBlur: function (data) 
    {
      var sectionId = data.sectionId;
      var section = this.getSectionById([this.currentMainSection], sectionId, false);
      section.lock.isLocked=false;
      // console.log('Unlocking SectionInFocus: ',section.text, section.lock.isLocked);
      // section.lock.lockedBy="";
    },    
    UpdateSection: function (data) 
    {
      if (data.documentId==this.currentJotId)
      {
  ...
  methods: 
  {
    ...
    saveSections()
    {
      ...
      var self = this;
      Operations.saveOperations(this.$store, this.$socket, function(errors, results)
      {
        if (errors)
        {
          console.log(errors);
    ...
    sectionBlurred(event, section, depth)
    {
      ...
      Comms.wsEmit(this.$socket, Common.WSTypes.SectionLostFocus, { jotId: this.currentJotId, sectionId: section.id} );
    },
